...
// ─── Export Excel & PNG พร้อมกัน ──────────────────────────────
async function exportExcelAndPNG() {
  if (typeof validateBeforeExport === "function" && !validateBeforeExport()) return;
  const qtno = gv('qt-no') || 'QT';
  const date = gv('qt-date') || new Date().toISOString().slice(0,10);
  ...
  setTimeout(() => {
    alert(
      `✅ Export สำเร็จ!\n\n` +
      `📊 Excel → ย้ายไปที่:\n   customers/${ym}/${qtno}.xlsx\n\n` +
      `🖼️ PNG → ย้ายไปที่:\n   png-customers/${ym}/Quotation_${qtno}.png`
    );
  }, 600);
}
